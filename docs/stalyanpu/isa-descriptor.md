...
| 13 | `param_base` | OC başına `{bias i32, mult u16, shift u8, pad}` 8 bayt |
...
bölüm 4 KB hizalı. Başlık alanları `isa.BLOB_HEADER`: magic, version, base, size, desc_off,
desc_count, param_off, param_size, scratch_base, scratch_size, input_off, input_bytes,
n_outputs, outputs_off, crc32. Çıkış tablosu girdileri (`isa.OUTPUT_ENTRY`): offset, h, w,
c, scale_q16, zp, name_off. Derleyici blob'u `--base` adresine bağlar; scratch bölgesi
(aktivasyonlar) `--scratch` ile ayrı verilir. Emitter M2'de yazılır.
